...
function getFill(context) {
  ...
  for (const trackedFile of trackedFiles) {
    ...
    let outputFormatToUse = settings2.outputFormat;
    if (settings2.separateOutputFormats) {
      switch (trackedFile.supposedList) {
        case "created":
          outputFormatToUse = settings2.createdFormat;
          break;
        case "modified":
          outputFormatToUse = settings2.modifiedFormat;
          break;
        case "deleted":
          outputFormatToUse = settings2.deletedFormat;
          break;
      }
    }
...
function getFinalNoteContent(fileContent, context) {
  ...
  for (const l in fill) {
    const listType = l;
    if (dividerPositions[listType].start === -1 || dividerPositions[listType].end === -1) {
      continue;
    }
  ...
  constructor() {
    ...
    this.replacements = [
      { template: "path", replaceWith: (file) => file.path },
      {
        ...
        replaceWith: (file) => {
          var _a;
          return getPlugin().app.fileManager.generateMarkdownLink(
            file,
...
function isNewNotePeriod(settings2) {
  ...
  let granularity = (/* @__PURE__ */ new Map([
    ["daily", "day"],
    ["weekly", "week"],
    ["monthly", "month"]
